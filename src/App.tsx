...
export const App: React.FC<{ config: AppConfig }> = ({ config }) => {
  ...
  useEffect(() => {
    ...
    if (autoOn) {
      bootMessages.push({
        tag: '[AUTO]',
        text: `automaton  ${link(autoLinked)}   ${autoStatus?.packageName ?? 'sovereign'} · laws ${autoStatus?.constitutionLaws.join('/') || '—'} · [7]`,
    ...
    bootMessages.push(
      ...
      { tag: '>>', text: 'type /help · TAB cycles · [7] AUTOMATON · ESC severs', color: '#E8E0DD' },
      { tag: '', text: '', color: '#3D0710' },
    );
