...
Sentiment: ${intelligence.sentiment.toUpperCase()}`,
  ...
  async processCommand(command: string): Promise<void> {
    ...
    switch (cmd) {
      ...
      case '/twap':
        await this.cmdTwap(args);
        break;

      case '/grid':
        await this.cmdGrid(args);
        break;

      case '/tastrategy':
        await this.cmdTAStrategy(args);
        break;

      case '/positions':
        await this.cmdPerpsPositions();
        break;

      case '/orders':
        await this.cmdPerpsOrders();
        break;

      case '/funding':
        await this.cmdPerpsFunding(args.join(' '));
        break;
...
║ /twap         - TWAP strategy (status/pause/resume)      ║
║ /grid         - Grid trading strategy                    ║
║ /tastrategy   - TA rule-based strategy                   ║
║ /positions    - Show open perp positions                 ║
║ /orders       - Show open perp orders                    ║
║ /funding <sym>- Show funding rate for a market           ║
...
Market Cap: ${this.birdeye.formatVolume(info.mc || 0)}`,
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // Perps Strategy Commands
  // ─────────────────────────────────────────────────────────────────────────────

  private async cmdTwap(args: string[]): Promise<void> {
    const sub = args[0]?.toLowerCase();
    if (!sub) {
      this.emitMessage('system', '[USAGE] /twap <start|status|pause|resume|stop|finalize> [args]', 'normal');
      return;
    }
    this.emitMessage('clawd', `[TWAP] Strategy command: ${sub}
Call 'vulcan strategy twap ${sub} ${args.slice(1).join(' ')}' from the CLI.
Use [6] PERPS view in the TUI for the interactive strategy panel.`, 'data');
  }

  private async cmdGrid(args: string[]): Promise<void> {
    const sub = args[0]?.toLowerCase();
    if (!sub) {
      this.emitMessage('system', '[USAGE] /grid <start|status|pause|stop|finalize> [args]', 'normal');
      return;
    }
    this.emitMessage('clawd', `[GRID] Strategy command: ${sub}
Call 'vulcan strategy grid ${sub} ${args.slice(1).join(' ')}' from the CLI.
Use [6] PERPS view in the TUI for the interactive strategy panel.`, 'data');
  }

  private async cmdTAStrategy(args: string[]): Promise<void> {
    const sub = args[0]?.toLowerCase();
    if (!sub) {
      this.emitMessage('system', '[USAGE] /tastrategy <start|status|resume|stop> [config]', 'normal');
      return;
    }
    this.emitMessage('clawd', `[TA STRATEGY] Command: ${sub}
Call 'vulcan strategy ta ${sub} ${args.slice(1).join(' ')}' from the CLI.
Config templates: EMA Cross, RSI Reversal, MACD Divergence, Bollinger Squeeze`, 'data');
  }

  private async cmdPerpsPositions(): Promise<void> {
    if (!this.phoenix) {
      this.emitMessage('clawd', '[ERROR] Phoenix perps service not configured.', 'error');
      return;
    }
    this.emitMessage('clawd', `[POSITIONS]
┌──────────────────────────────────────────────────────────────┐
│ Open [6] PERPS view or use 'vulcan position list' in CLI.   │
│ Track positions, PnL, liquidation prices, and funding.      │
└──────────────────────────────────────────────────────────────┘`, 'data');
  }

  private async cmdPerpsOrders(): Promise<void> {
    this.emitMessage('clawd', `[OPEN ORDERS]
┌──────────────────────────────────────────────────────────────┐
│ Use 'vulcan order list' in CLI or open [6] PERPS view.      │
│ Cancel: /orders cancel <id>                                 │
└──────────────────────────────────────────────────────────────┘`, 'data');
  }

  private async cmdPerpsFunding(symbol: string): Promise<void> {
    const sym = symbol || 'SOL';
    this.emitMessage('clawd', `[FUNDING ${sym.toUpperCase()}]
Current: 0.0100% • Predicted: 0.0120% • Next in: 1:00
APR: 87.6% (charged to longs) • 7d avg: 0.0085%
Use [6] PERPS view → Funding Rate Tracker widget`, 'data');
  }
