    ...
    try {
      const response = await fetch(`${this.baseUrl}/chat/completions`, {
        ...
        headers: {
          ...
          'X-Title': `${OPENCLAWD_SITE_NAME} Dark Clawd`,
          'X-OpenRouter-Title': `${OPENCLAWD_SITE_NAME} Dark Clawd`,
        },
        body: JSON.stringify({
          model: this.model,
          messages,
  ...
  async analyzeWithReasoning(prompt: string, context = ''): Promise<AIResponse | null> {
    return this.chat(
      [
        {
          role: 'system',
          content: `You are DARK CLAWD, an OpenClawd recursive autonomous AI agent analyzing Solana cryptocurrency markets.
