...
/**
 * Vendored Automaton sovereign runtime (./automaton).
 * Paths are relative to the Dark Clawd package root unless absolute.
 */
export const AutomatonConfigSchema = z.object({
  enabled: z.boolean().default(true),
  /** Directory name or absolute path for the automaton package */
  packageDir: z.string().default('automaton'),
  showInBoot: z.boolean().default(true),
  /** Surface Automaton view in Bloomberg nav ([7]) */
  tuiViewEnabled: z.boolean().default(true),
  /** Conway API URL override for future runtime spawn */
