...
type ViewMode = 'market' | 'trading' | 'portfolio' | 'analytics' | 'agent' | 'automaton';
...
export const BloombergDashboard: React.FC<BloombergDashboardProps> = ({
  ...
  useInput((input, key) => {
    ...
    if (input === '6' && automatonViewEnabled) setView('automaton');
...
const BottomBar: React.FC<{
  ...
  if (automatonViewEnabled) {
    views.push({ key: '6', view: 'automaton', label: 'AUTOMATON' });
  }
      ...
      <Box>
        ...
        <Box flexDirection="column" width={colWidth} paddingLeft={1}>
          <Box borderStyle="round" borderColor="cyan" padding={1} flexDirection="column">
            ...
            <Text color="yellow">clawd automaton status</Text>
            ...
            {onCommand && (
              <Box marginTop={1}>
                <Text color="green">Tip: type /automaton in Agent view</Text>
              </Box>
            )}
          </Box>
        </Box>
